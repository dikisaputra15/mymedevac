...
<style>
    ...
    .p-modal{
     ...
     .btn-danger{
        ...
        /* ===== Filter panel: province dropdown ===== */
        .select-input {
    ...
    (data || []).forEach(airport => {
        ...
        const popupContent = `
            ...
            <strong>Address:</strong>
                ...
                ${airport.province_name ? ', ' + airport.province_name : ''}, Malaysia <br>
...
async function applyAirportFilters() {
    ...
    // /api/airports mengembalikan array polos.
    // Tetap toleran kalau suatu saat endpoint mengirim { airports, categoryCounts }.
    const airports       = Array.isArray(result) ? result : (result.airports || []);
    const serverCounts   = (!Array.isArray(result) && result.categoryCounts) ? result.categoryCounts : null;
    ...
    const filteredAirports = airports.filter(a => {
        ...
        // FILTER POLYGON
        // =========================
        // Server sudah memfilter lewat parameter "polygon"; ini lapis kedua
        // di sisi klien. Dilewati kalau turf gagal dimuat supaya filter
        // kategori & radius tetap jalan.
        if (drawnPolygonGeoJSON && typeof turf !== 'undefined') {
